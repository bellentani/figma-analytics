...
async function generateReportSummary(reportData, libraryName, startDate, endDate, executionStartTime) {
    const summary = {
    ...
    // Gerar MD
    await generateSummaryMD(summary);
...
async function addSummaryToCSV(summary, reportData) {
    // Adiciona linhas em branco e o sumário no início do CSV
    const summaryRows = [
        ['REPORT SUMMARY'],
        ...
        [], // Linha em branco
        [], // Linha em branco
        ['COMPONENT DATA'] // Cabeçalho para os dados dos componentes
    ];

    // Combinar o sumário com os dados existentes
    return [...summaryRows, ...reportData];
}

async function generateSummaryMD(summary) {
...
`;
    ...
    // Salvar o arquivo MD
    const mdFileName = `report_summary_${moment().format('YYYY-MM-DD-HH-mm')}.md`;
    await fs.promises.writeFile(`${REPORTS_DIR}/${mdFileName}`, mdContent);
}
