...
if (!fs.existsSync(REPORTS_DIR)) {
    ...
    console.log(`Diretório ${REPORTS_DIR} criado com sucesso`);
}
...
async function generateConsolidatedReport(allReportData, period) {
    try {
        console.log('\n=== INICIANDO GERAÇÃO DO RELATÓRIO CONSOLIDADO ===');
        console.log(`Quantidade de dados a serem processados: ${allReportData.length}`);
        ...
        // Adiciona a data de criação do relatório para cada item
        ...
        console.log(`Nome do arquivo a ser gerado: ${fileName}`);
        ...
        console.log('Iniciando escrita do arquivo CSV...');
        await csvWriter.writeRecords(enrichedReportData);
        console.log(`✓ Relatório consolidado gerado com sucesso em: ${REPORTS_DIR}/${fileName}`);
...
(async () => {
    ...
    try {
        ...
        console.log('Gerar relatório consolidado:', flags.has('consolidated'));
        ...
        // Gerar relatório consolidado apenas se a flag --consolidated estiver presente
        if (flags.has('consolidated')) {
            console.log('\n=== VERIFICAÇÃO DO RELATÓRIO CONSOLIDADO ===');
            console.log('Flag --consolidated detectada:', flags.has('consolidated'));
            console.log(`Quantidade de dados coletados: ${allReportData.length}`);
            
            if (allReportData.length === 0) {
                console.warn('⚠️ Nenhum dado coletado para gerar o relatório consolidado');
            } else {
                try {
                    await generateConsolidatedReport(allReportData, params.period || '30d');
                    console.log('✓ Processo de geração do relatório consolidado finalizado');
                } catch (error) {
                    console.error('✗ Erro ao gerar relatório consolidado:', error);
                }
            }
        } else {
            console.log('\nRelatório consolidado não solicitado (--consolidated não presente)');
        }
